export function PaletteC() {
  const bg = "#F4F2EE";
  const card = "#FFFFFF";
  const receiveBtn = "#DAEEF8";
  const receiveBtnIcon = "#4A8EB0";
  const sendBtn = "#F2D9CC";
  const sendBtnIcon = "#C47052";
  const txLogBg = "#FFFFFF";
  const txSent = "#F0CCBC";
  const heading = "#1C2A36";
  const muted = "#7B8C9A";
  const amount = "#C45D3E";
  const green = "#22C55E";
  ...
  return (
    <div style={{ width: 320, minHeight: 620, background: bg, borderRadius: 32, padding: 24, fontFamily: "'Nunito', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: 12, left: 12, background: "rgba(0,0,0,0.06)", borderRadius: 8, padding: "4px 6px", fontSize: 10, color: "#888", fontWeight: 700 }}>C — Cool Linen</div>
      ...
      <div style={{ background: txLogBg, borderRadius: 20, marginTop: 28, padding: 20, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
        ...
        {[{ label: "Sent payment", time: "Yesterday, 11:37 PM", amt: "-100 sats", fee: "3" },
          ...
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderTop: i > 0 ? "1px solid #EBE8E2" : "none" }}>
