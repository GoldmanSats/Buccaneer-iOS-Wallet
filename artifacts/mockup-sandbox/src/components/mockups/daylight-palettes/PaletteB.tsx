export function PaletteB() {
  const bg = "#F7F4EE";
  const card = "#FFFFFF";
  const receiveBtn = "#E0F0E8";
  const receiveBtnIcon = "#4A9B6E";
  const sendBtn = "#FADDD2";
  const sendBtnIcon = "#D47B5E";
  const txLogBg = "#FFFFFF";
  const txSent = "#F5CFC0";
  const heading = "#2A1F14";
  const muted = "#8C7E72";
  const amount = "#B85A3A";
  const green = "#22C55E";
  ...
  return (
    <div style={{ width: 320, minHeight: 620, background: bg, borderRadius: 32, padding: 24, fontFamily: "'Nunito', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: 12, left: 12, background: "rgba(0,0,0,0.06)", borderRadius: 8, padding: "4px 6px", fontSize: 10, color: "#888", fontWeight: 700 }}>B — Sage & Terra</div>
      ...
      <div style={{ background: txLogBg, borderRadius: 20, marginTop: 28, padding: 20, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
        ...
        {[{ label: "Sent payment", time: "Yesterday, 11:37 PM", amt: "-100 sats", fee: "3" },
          ...
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderTop: i > 0 ? "1px solid #EDE9E0" : "none" }}>
