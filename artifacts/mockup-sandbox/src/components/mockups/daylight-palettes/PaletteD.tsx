export function PaletteD() {
  const bg = "#F6F3ED";
  const card = "#FFFFFF";
  const receiveBtn = "#D4E8F0";
  const receiveBtnIcon = "#3D8DAE";
  const sendBtn = "#F8DDD0";
  const sendBtnIcon = "#CC6E4E";
  const txLogBg = "#FFFFFF";
  const txSent = "#F0D0C0";
  const heading = "#1E2D3D";
  const muted = "#8A95A0";
  const amount = "#CC5533";
  const green = "#22C55E";
  ...
  return (
    <div style={{ width: 320, minHeight: 620, background: bg, borderRadius: 32, padding: 24, fontFamily: "'Nunito', sans-serif", position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "absolute", top: 12, left: 12, background: "rgba(0,0,0,0.06)", borderRadius: 8, padding: "4px 6px", fontSize: 10, color: "#888", fontWeight: 700 }}>D — Maritime Soft</div>
      ...
      <div style={{ background: txLogBg, borderRadius: 20, marginTop: 28, padding: 20, boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
        ...
        {[{ label: "Sent payment", time: "Yesterday, 11:37 PM", amt: "-100 sats", fee: "3" },
          ...
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderTop: i > 0 ? "1px solid #ECE8E0" : "none" }}>
